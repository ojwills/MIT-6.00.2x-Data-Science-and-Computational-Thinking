...
def r_squared(y, estimated):
    ...
    meanError = error/N
    RSquareError = 1 - (meanError/np.var(y))
    #print(RSquareError)
    return RSquareError

# Problem 3
def evaluate_models_on_training(x, y, models, city):
    """
    ...
    for i in range(len(models)):
        ...
        pylab.title('Average temperature in USA cities')
    pylab.show()
...
x1 = INTERVAL_1
x2 = INTERVAL_2
for city in CITIES[2:4]: # set up to look at San Diego and Philadellphia
    y = []
    for year in INTERVAL_1:
        y.append(np.mean(raw_data.get_yearly_temp(city, year)))
    d = [1]
    models = generate_models(x1, y, d)    
    evaluate_models_on_training(x1, y, models, city)